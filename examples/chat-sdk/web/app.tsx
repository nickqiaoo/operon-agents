// Browser chat client. The sidebar lists managed sessions -- the only conversation store
// there is -- and picking one replays its transcript from the session's event log
// (/api/history). `useChat` POSTs the active conversation to /api/chat and reads the AI SDK
// message stream off the response; the server holds that response open for the whole agent
// turn, and every `assistant.delta` the session streams lands here as its own text-delta, so
// the brief types itself out. /api/activity is a second, message-free stream: the tool calls,
// model requests, and retries the bridge observes while it works.
...
function BriefCard({ stats }: { stats: BriefStats }) {
  return (
    <aside className="brief-card">
      ...
      <a href={eventsPath(stats.sessionId)} target="_blank" rel="noreferrer">
        Open session events
      </a>
    </aside>
  );
...
function Conversation({
  ...
  // The whole turn is one response: "streaming" covers the minutes between the
  // acknowledgment and the brief, so it doubles as the progress indicator.
    ...
    <>
      ...
      <form
        onSubmit={(event) => {
          ...
          if (!text || busy) return;
          setInput("");
          // The first message of a fresh conversation doubles as its title: the
          // bridge writes it onto the managed session, so the sidebar shows it too.
      ...
      >
        ...
        {busy ? (
          ...
          // server-side, and a re-sent message queues behind it (see README.md).
    ...
    <div className="app">
      <aside className="sidebar">
        <header>
          ...
          <p>Chat SDK × operon managed agents</p>
        </header>
        ...
        <p className="sidenote">
          Each conversation is a managed session. The chat server stores nothing: restart it, and
