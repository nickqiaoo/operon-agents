...
| [`chat-sdk`](./chat-sdk) | A **chat bot** on the managed server: Vercel's Chat SDK (web adapter; Slack/Teams/Telegram are one adapter away) bridged to `operon-managed-agents` -- streamed replies, tool-call traces, transcript replay from the session log. A port of Anthropic's Chat SDK quickstart. | HTTP server + web page |
