...
//    to session(). A value import bundles the whole framework into your artifact and creates the
...
//    the cleanup function returned by session(), or be a registration such as api.on / api.registerTool
...
//    definition and read services["service-name"] in session(api, { services }) — the framework
...
//    harness half publishes itself, and its id is the service name (see examples/peers-extension).
//
...
const plugin: ExtensionDefinition = {
  ...
  id: "example-plugin",
  session(api) {
    // Scoping rule: one definition is reused across sessions and session() runs once per session, so
    // per-session state is declared inside session() (as below). Declaring it at module level shares it
    ...
    // Rule 2: whatever session() opened is closed here.
