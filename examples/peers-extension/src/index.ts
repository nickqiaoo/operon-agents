...
// process-level sharing" (an extension with a create half).
//
// The whole file is a single `peers(...)` call: it returns an ordinary ExtensionDefinition with a
// create half, the same definition a server passes to `createHarness({ extensions: [peers(...)] })`.
// One body of code, two delivery paths. The create half builds the shared network and the framework
// registers it as the "peers" service; the setup half decides from the session params whether to
...
//
// State lives in the data directory the framework provides (host.dataDir, outside the bundle
// directory), so swapping the network instance on reload and overwriting the bundle on update both
...
export default peers({
  ...
  // options) for sessions of that kind; the create half's spawn factory appends the Hub mount and
