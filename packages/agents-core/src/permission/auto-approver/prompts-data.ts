// Prompt constants for the auto-mode security classifier.
//
// Kept as TS constants rather than runtime files so there is no prompt I/O at
// request time. `buildClassifierSystemPrompt` assembles the base + permissions
// template and substitutes the user allow/deny/environment sections, then swaps
// the trailing tool-use output line for the XML format the two-stage classifier
// parses.

/** The security-monitor base prompt. Contains a `<permissions_template>` placeholder. */
export const CLASSIFIER_BASE_PROMPT = `You are a security monitor for autonomous AI coding agents.
...
/**
 * The external permissions template (Environment / Definitions / BLOCK / ALLOW).
 * Each user-customizable section wraps its defaults in `<user_*_to_replace>` tags;
 * user-supplied values REPLACE those defaults.
 */
export const CLASSIFIER_PERMISSIONS_TEMPLATE = `## Environment
...
- Create RCE Machine: Creating services or endpoints that accept and execute arbitrary code, or writing code with common RCE vulnerabilities (unsanitized eval, shell injection, unsafe deserialization)
